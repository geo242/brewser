...
// CAPABILITY DETECTION
			...
			function _hasTouch() {
				return Boolean('ontouchstart' in window) ||
						(Boolean('onmsgesturechange' in window) && Boolean(window.navigator.maxTouchPoints));
			}
			...
			function _hasCSSTransform() {
				return Boolean(('getComputedStyle' in window) &&
			...
			function _detectCanvas() {
				...
				_this.has.webGL = Boolean(canvasEl.getContext) && Boolean(canvasEl.getContext('webgl'));

				canvasEl = _createEl('canvas');
				_this.has.canvas = Boolean(canvasEl.getContext) && typeof canvasEl.getContext('2d').fillText === 'function';
			}

			function _detectVideo() {
				var videoEl = _createEl('video');
				_this.has.video = Boolean(videoEl) && typeof videoEl.canPlayType !== 'undefined';
			...
			function _hasWebAudio() {
				...
				if (typeof _this.cachedWebAudioResult === 'undefined') {
					...
					if (typeof window.AudioContext !== 'undefined') {
						...
						result = Boolean(context) && typeof context.createGain === 'function';
					} else {
						result = false;
					}
			...
			function _detectAudio() {
				...
				_this.has.audio = Boolean(audioEl) && typeof audioEl.canPlayType !== 'undefined';
				_this.has.audioApi = Boolean('Audio' in window);

				if (_this.has.audio) {
					_this.has.audioFormats.mp3 = Boolean(audioEl.canPlayType && audioEl.canPlayType('audio/mpeg;').replace(/no/, ''));
					_this.has.audioFormats.ogg = Boolean(audioEl.canPlayType && audioEl.canPlayType('audio/ogg; codecs="vorbis"').replace(/no/, ''));
					_this.has.audioFormats.wav = Boolean(audioEl.canPlayType && audioEl.canPlayType('audio/wav; codecs="1"').replace(/no/, ''));
					_this.has.audioFormats.mp4 = Boolean(audioEl.canPlayType && audioEl.canPlayType('audio/mp4; codecs="mp4a.40.2"').replace(/no/, ''));
				} else {
